...
## 2. Usage
 We incorporate the training and evaluation of SoftProto in the **train_softproto.py**. Just run it as below. The ```--lm``` argument is used to specify the type of pre-trained language models.

```
CUDA_VISIBLE_DEVICES=0 python train_softproto.py --dataset res14 --lm external --seed 123
```

Or you can run the shell script ```sh res14_run.sh``` to get all results of a certain dataset.
...
We have re-run the experiments on another machine which is different from the one used in our paper. The results are listed as below. The corresponding log files are contained in the ```./log/``` folder.
